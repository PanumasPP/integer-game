...
const learningContentData = {
    addition: {
        title: "ทบทวน: การบวกจำนวนนับ",
        content: `<h3><i class="fas fa-plus-circle text-green-500 mr-2"></i>การบวกจำนวนนับ (ผลลัพธ์ไม่เกิน 1,000)</h3>
                  <p>การบวกเลขคือการรวมจำนวนตั้งแต่สองจำนวนขึ้นไปเข้าด้วยกัน เราจะเริ่มบวกจาก<b>หลักหน่วย (ขวาสุด)</b> ไปยังหลักสิบ และหลักร้อย (ซ้ายสุด) ตามลำดับ</p>
                  
                  <h4>1. การบวกแบบไม่มีตัวทด</h4>
                  <p>เป็นการบวกที่ผลรวมของเลขในแต่ละหลักไม่ถึง 10</p>
                  <div class="explanation-box"><b>ตัวอย่าง:</b> 152 + 34
                      <pre class="mt-2 text-lg font-mono text-blue-700">
  152
+  34
-----
  186</pre>
                      <p><b>ขั้นตอน:</b><br>
                      1. <b>หลักหน่วย:</b> 2 + 4 = 6<br>
                      2. <b>หลักสิบ:</b> 5 + 3 = 8<br>
                      3. <b>หลักร้อย:</b> 1 + 0 = 1<br>
                      ดังนั้น คำตอบคือ 186</p>
                  </div>

                  <h4>2. การบวกแบบมีตัวทด</h4>
                  <p>เมื่อผลบวกของเลขในหลักใดมีค่าตั้งแต่ 10 ขึ้นไป เราจะต้องนำเลขหลักสิบไป "ทด" ไว้ในหลักถัดไปทางซ้าย</p>
                  <div class="explanation-box"><b>ตัวอย่าง:</b> 248 + 135
                      <pre class="mt-2 text-lg font-mono text-blue-700">
...
-----
  383</pre>
                      <p><b>ขั้นตอน:</b><br>
                      1. <b>หลักหน่วย:</b> 8 + 5 = 13 (ใส่ 3 ที่หลักหน่วย, นำ 1 ไปทดที่หลักสิบ)<br>
                      2. <b>หลักสิบ:</b> 4 + 3 + <span class="text-red-500">1 (ตัวทด)</span> = 8<br>
                      3. <b>หลักร้อย:</b> 2 + 1 = 3<br>
                      ดังนั้น คำตอบคือ 383</p>
                  </div>`
    },
};
...
function generateAndShowReport() {
    ...
    let html = `<table class="w-full text-sm text-left text-gray-500">
        <thead class="text-xs text-gray-700 uppercase bg-gray-100">
            <tr>
                ...
                <th scope="col" class="px-6 py-3 text-center">การบวก (ง่าย)</th>
                <th scope="col" class="px-6 py-3 text-center">การบวก (กลาง)</th>
                <th scope="col" class="px-6 py-3 text-center">การบวก (ยาก)</th>
            </tr>
        </thead><tbody>`;
    ...
    players.forEach(player => {
        ...
        player.performanceLog.forEach(log => {
            if(log.category === 'addition') {
                totalAnswered++;
                if (log.correct) totalCorrect++;
                if (stats[log.difficulty]) {
                    stats[log.difficulty].total++;
                    if (log.correct) stats[log.difficulty].correct++;
                }
            }
        });
        ...
        categories.forEach(cat => {
            html += `<td class="px-6 py-4 text-center">${stats[cat].correct}/${stats[cat].total}</td>`;
        });
        html += `</tr>`;
    });
    html += `</tbody></table>`;
...
function startLearningQuiz(category) {
    ...
    const allCategoryQuestions = allCards.puzzle.medium.filter(q => q.category === category);
